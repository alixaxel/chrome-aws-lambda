...
Super.prototype.newPage = async function (...hooks) {
  ...
  await result
    .browser()
    .userAgent()
    .then((agent) => result.setUserAgent(agent.replace('Headless', '')));
    ...
    window.σ = {
      ...
      $number: function (data, decimal = null, index = null, property = 'textContent') {
        ...
        if (typeof data === 'string') {
          ...
          if (result != null) {
            result = result.map((value) => parseFloat(value.replace(new RegExp(`[^-+0-9${decimal}]+`, 'g'), '').replace(decimal, '.')));
      ...
      $string: function (data, property = 'textContent') {
        ...
        if (typeof data === 'object') {
          ...
          if (Array.isArray(data) === true) {
            return data.map((value) => σ.$string(value, property));
          }
